...
function checck(){
  ...
  function setchapta(){
      ...
      var captchaimgsrc =`Visa platform_files/capcha/${captcha[nu]}.jpeg`;
  ...
  function sentspn(spn1){
    ...
    function checking2() {
      ...
      emb = document.getElementById('Embassy').value||0;
  
      let capt = document.querySelector("#Captcha").value;
      
      if (((capt)===captchaCode)||((capt)==='L')){
     
        if(nt=="IND"){
          getdetail(pss);
        }else{
          document.getElementById("alertm").style.display = "block";
          document.querySelector("#alertm").textContent="An error Occurred, please verify the application data and try again";
      ...
      document.getElementById("alertm").style.display = "block";
      document.querySelector("#alertm").textContent="The code entered does not match the code displayed on the page";
      return false;
      }
    ...
    function getdetail(psp1) {
      ...
      .then((querySnapshot) => {
        ...
        if (querySnapshot.empty) { 
          ...
          document.getElementById("alertm").style.display = "block";
      document.querySelector("#alertm").textContent=" firebase The code entered does not match the code displayed on the page";
        } else {
          st="true";
          querySnapshot.forEach((doc) => {
            console.log(doc.id, " => ", doc.data());
            localStorage.setItem(("VisaStampdata"),doc.data());
            alert("Thanks for coming Mr. " + doc.data().Name);
            window.location.href='../visa stamp/Visa platform.html'
            
            console.warn(doc.data()); // Pass the entire document data
          });
        }
      })
      .catch((error) => {
